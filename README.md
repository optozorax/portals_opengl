...
Можно перенаправить рисование не на экран, а в этот фреймбуфер. Обычно это используются для пост-обработки, или чтобы показать как выглядит буфер глубины.
...
Или, например, у нас могут быть порталы, которые преобразуют всю сцену таким образом, что полигоны, ориентированные по часовой стрелке, вдруг становятся ориентированными против часовой стрелки О_о. Для этого используется специальный флаг clockWiseInvert при рендеринге порталов.
